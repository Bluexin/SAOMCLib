...
class PartyServerObject(override var leaderInfo: PlayerInfo) : IParty() {
    ...
    private fun remove(player: PlayerInfo): Boolean {
        ...
        while (members.hasNext()) {
            ...
            if (memeber == player) {
                members.remove()
                Type.LEAVE.updateClient(player.player as ServerPlayerEntity, this, player)
    ...
    override fun sync(member: PlayerInfo) {
        Type.REFRESH.updateClient(member.player as ServerPlayerEntity, this, PlayerInfo.EMPTY)
    }
