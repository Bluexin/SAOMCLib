...
object PacketPipeline {
    ...
    /**
     * Sends a packet to all the players around a certain [NetworkRegistry.TargetPoint] (from the server!)
     * @see cpw.mods.fml.common.network.simpleimpl.SimpleNetworkWrapper.sendToAllAround
     */
    fun sendToAllAround(message: IMessage, point: NetworkRegistry.TargetPoint) = ntw.sendToAllAround(message, point)

    /**
     * Sends a packet to all the players around certain coordinates, within range (from the server!)
     * @see cpw.mods.fml.common.network.simpleimpl.SimpleNetworkWrapper.sendToAllAround
     */
    fun sendToAllAround(message: IMessage, dimension: Int, x: Double, y: Double, z: Double, range: Double) = sendToAllAround(message, NetworkRegistry.TargetPoint(dimension, x, y, z, range))

    /**
     * Sends a packet to all the players around certain player, within (from the server!)
     * @see cpw.mods.fml.common.network.simpleimpl.SimpleNetworkWrapper.sendToAllAround
     */
    fun sendToAllAround(message: IMessage, player: EntityPlayer, range: Double) = sendToAllAround(message, player.world.provider.dimensionId, player.posX, player.posY, player.posZ, range)
